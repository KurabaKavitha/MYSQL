...
INSERT INTO CHAMPINES VALUES(1,'LAWNBOWLS',11,'INDIA',6,'RUPARANI',15,10,'UKCOMPANY','BOWRINGINS',1,21,,10,1,'CHOUBEY','PINKI','LOYCHOUBEY','KINGPINS','INDIA','HOLYROLLERS','MORTALPINS','FLATSUR','somashker',6584265,'INDIA5452GH89',current_date(),current_time(),now(),now(),'Arjun','male');
...
SELECT*FROM CHAMPINES WHERE game_name='HOCKEY' and id=1;
SELECT*FROM CHAMPINES WHERE gender_name='male' and id=4;
SELECT*FROM CHAMPINES WHERE game_name between' WEIGHTLEFT' and 'ATHLE';
SELECT*FROM CHAMPINES WHERE ID IN(3,4,5);
SELECT*FROM CHAMPINES WHERE ID NOT IN(2,1,4,5);
SELECT *FROM CHAMPINES ORDER BY game_name;
SELECT *FROM CHAMPINES ORDER BY caption_name;
SELECT count(game_name) from  CHAMPINES group by caption_name having count(game_name);
SELECT count(id)FROM CHAMPINES group by id;
SELECT SUM(id) from CHAMPINES group by id;
SELECT max(id) from CHAMPINES group by id;
SELECT min(id) from CHAMPINES group by id;
SELECT avg(id) from CHAMPINES group by id;
SELECT* FROM CHAMPINES WHERE game_name between 1 and 4;
SELECT* FROM CHAMPINES WHERE game_name between 2 and 5;
SELECT* FROM CHAMPINES WHERE game_name between 8 and 11;
SELECT* FROM CHAMPINES WHERE game_name between 4 and 6;
SELECT* FROM CHAMPINES WHERE game_name between 5 and 10;

SELECT*FROM  CHAMPINES order by game_name ;
SELECT  caption_name FROM  CHAMPINES order by caption_name ;
SELECT  sponcer FROM  CHAMPINES order by sponcer ;
SELECT  venue FROM  CHAMPINES order by venue;
SELECT  country_participated FROM  CHAMPINES order by country_particapated ;
SELECT  qualifier_team_runner FROM  CHAMPINES order by qualifier_team_runner ;
SELECT  game_type FROM  CHAMPINES order by game_type ;
